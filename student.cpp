...
ssize_t readFromFile(std::vector<Student>& databaseFile, const std::string fileName)
{
    ...
    while(i < count)
    {
        ...
        addStudent(databaseFile, Student(
            temp_all.substr(temp_all.find(fields[0]) +fields[0].size(), temp_all.find(fields[1])-temp_all.find(fields[0]) - fields[0].size())
            , temp_all.substr(temp_all.find(fields[1])+fields[1].size(), temp_all.find(fields[2]) - temp_all.find(fields[1]) - fields[1].size())
            , temp_all.substr(temp_all.find(fields[2])+fields[2].size(), temp_all.find(fields[3])- temp_all.find(fields[2]) -fields[2].size())
            , temp_all.substr(temp_all.find(fields[3])+fields[3].size(), temp_all.find(fields[4])-temp_all.find(fields[3]) - fields[3].size())
            , temp_all.substr(temp_all.find(fields[4])+fields[4].size(), temp_all.find(fields[5]) - temp_all.find(fields[4]) - fields[4].size())
            , temp_all.substr(temp_all.find(fields[5])+fields[5].size(), temp_all.find(fields[0],fields[0].size()) - temp_all.find(fields[5]) - fields[5].size())
        ));
        //erase read data
